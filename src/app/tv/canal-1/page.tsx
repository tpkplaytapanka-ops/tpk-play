...
export default function Canal1Page() {
  ...
  const fetchConfig = useCallback(async () => {
    try {
      const res = await fetch('/api/broadcast/tv1')
      if (res.ok) {
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 sm:px-6 py-6">
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            <h1 className="text-2xl md:text-3xl font-bold text-white">Señal Colombia</h1>
            {config?.isActive && (
              <Badge className="bg-green-600 hover:bg-green-700">EN VIVO</Badge>
            )}
          </div>
          <p className="text-zinc-400">Canal público nacional de Colombia</p>
        </div>
        ...
        {loading ? (
            ...
            <Card className="bg-zinc-900/50 border-zinc-800">
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-white font-medium">{config.displayName || 'Señal Colombia'}</h3>
                    <p className="text-zinc-500 text-sm">Canal público nacional</p>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="w-full aspect-video bg-zinc-900 rounded-xl flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-zinc-500">Este canal no está disponible en este momento.</p>
          </div>
        )}
      </main>
