...
export default function Canal7Page() {
  ...
  const fetchConfig = useCallback(async () => {
    try {
      const res = await fetch('/api/broadcast/tv7')
      if (res.ok) {
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 sm:px-6 py-6">
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            <h1 className="text-2xl md:text-3xl font-bold text-white">{config?.displayName || 'Noticias Caracol'}</h1>
            {config?.isActive && (
              <Badge className="bg-red-600 hover:bg-red-700">EN VIVO</Badge>
            )}
          </div>
          <p className="text-zinc-400">{config?.description || 'Noticias Caracol en vivo'}</p>
        </div>
            ...
            <Card className="bg-zinc-900/50 border-zinc-800">
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-white font-medium">{config.displayName || 'Noticias Caracol'}</h3>
                    <p className="text-zinc-500 text-sm">{config.description || 'Noticias Caracol en vivo'}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
